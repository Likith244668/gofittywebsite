...
export default function OurServices() {
  ...
  return (
    <section
      ...
      ref={sectionRef}
    >
      <div className="mx-auto flex max-w-6xl flex-col gap-12 text-white lg:flex-row lg:items-start">
        {/* Main featured service card */}
        <div className="flex flex-col gap-8 lg:w-[40%]">
          <article
            className="group relative flex min-h-[500px] w-full flex-col items-center justify-center overflow-hidden rounded-[56px] bg-gray-900/10 ring-1 ring-white/10 shadow-[0_12px_40px_rgba(0,0,0,0.25),0_30px_80px_rgba(251,86,7,0.12)] transition-transform duration-300 ease-out hover:-translate-y-4"
            style={{
          ...
          >
            ...
            <PlusBadge />
            <span className="text-[42px] tracking-[0.28rem] text-white">
              {heroService.name}
            </span>
          </article>
        ...
        <div className="flex w-full gap-6 overflow-x-auto pb-4 lg:w-[60%] lg:overflow-visible">
          {verticalServices.map((service) => (
            <article
              key={service.name}
              className={`group relative flex h-[540px] min-w-[210px] flex-1 items-end justify-center overflow-hidden bg-gray-900/10 ring-1 ring-white/10 shadow-[0_10px_28px_rgba(0,0,0,0.25),0_24px_60px_rgba(251,86,7,0.10)] transition-transform duration-300 ease-out hover:-translate-y-4 ${service.radii}`}
              style={{
            ...
            >
              ...
              <PlusBadge />
              <span
                className={`absolute ${service.textSide} bottom-16 flex h-[70%] items-end text-[32px] tracking-[0.44rem] text-white`}
                style={{
